// pages/SettlementEdit.js - Settlement Edit Form
...
const SettlementEdit = () => {
  ...
  // Settlement data
  const [originalSettlement, setOriginalSettlement] = useState(null);
  ...
  const fetchSettlement = useCallback(async () => {
    try {
      ...
      for (const endpoint of endpoints) {
        try {
          console.log(`🔄 Trying settlement endpoint: ${endpoint}`);
          ...
          if (response.ok) {
            ...
            console.warn(`⚠️ Settlement endpoint ${endpoint} failed:`, response.status);
          }
        } catch (err) {
          console.warn(`⚠️ Settlement endpoint ${endpoint} error:`, err.message);
          continue;
        }
      }
      ...
      const settlement = settlementData.data?.settlement || settlementData.data;
      
      if (!settlement) {
        throw new Error('Settlement data not found');
      }

      console.log('📋 Loaded settlement:', settlement);
      setOriginalSettlement(settlement);
      ...
      setFormData({
        ...
        bill_amount: settlement.bill_amount || '',
        commission_percentage: settlement.commission_percentage || '',
        ...
        pending_amount: settlement.pending_amount || 0,
        extra_paid_amount: settlement.extra_paid_amount || 0,
        settlement_status: settlement.settlement_status || 'pending',
        payment_method: settlement.payment_method || '',
        payment_reference: settlement.payment_reference || '',
        bank_account: settlement.bank_account || '',
        tax_amount: settlement.tax_amount || 0,
        processing_fee: settlement.processing_fee || 0,
      ...
      if (settlement.user_first_name || settlement.user_last_name || settlement.user_email) {
        const user = {
          id: settlement.user_id,
          first_name: settlement.user_first_name || '',
          last_name: settlement.user_last_name || '',
      ...
      // Set selected store if store data exists
      if (settlement.store_name || settlement.store_email) {
        const store = {
          id: settlement.store_id,
          name: settlement.store_name || '',
          email: settlement.store_email || ''
        };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('🔄 Updating settlement:', submitData);
      ...
      if (result.success) {
        ...
        throw new Error(result.message || 'Settlement update failed');
      }
  ...
  if (error && !originalSettlement) {
    return <ErrorState message={`Failed to load settlement: ${error}`} />;
  }
    ...
    <div className="page-container" style={{paddingBottom: '100px'}}>
      ...
      <div style={{marginBottom: 'var(--spacing-lg)', display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)'}}>
        ...
        <span style={{color: 'var(--text-primary)', fontSize: '0.875rem'}}>
          Edit Settlement #{formData.settlement_id}
        </span>
      </div>
      ...
      <div className="chart-card">
        <div className="chart-header">
          <h3 className="chart-title">Edit Settlement</h3>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{padding: 'var(--spacing-lg)'}}>
          ...
          <div style={{marginBottom: 'var(--spacing-xl)'}}>
            ...
            <div className="form-group" style={{marginBottom: 'var(--spacing-lg)'}}>
              <label className="form-label">Settlement ID</label>
              <input
                type="text"
                name="settlement_id"
