// pages/RewardEdit.js - Edit Reward Page
...
const RewardEdit = () => {
  ...
  const fetchRewardData = async () => {
    try {
      setError(null);
      console.log(`🔄 Fetching reward data for editing, ID: ${id}`);
      ...
      console.log(`📡 Reward edit response status: ${response.status}`);
      ...
      console.log('✅ Reward Edit API Response:', data);

      if (!data.success || !data.data || !data.data.reward) {
        throw new Error('Invalid API response format');
      }

      const rewardData = data.data.reward;
      
      if (!rewardData.id || rewardData.id != id) {
      ...
      setFormData({
        ...
        reward_type: rewardData.reward_type || 'signup',
        normal_users_reward_value: rewardData.normal_users_reward_value || '',
        vip_users_reward_value: rewardData.vip_users_reward_value || '',
        is_active: rewardData.is_active || 1
      });
      
      console.log('✅ Reward edit data loaded successfully for reward:', rewardData.id);

    } catch (error) {
      console.error(`❌ Failed to fetch reward ${id} for editing:`, error.message);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(`🔄 Updating reward ${id} with data:`, updateData);
      ...
      if (result.success) {
        ...
        navigate(`/rewards/${id}`); // Navigate to details page
      } else {
  ...
  if (error && (!formData.reward_name && !formData.reward_type)) {
    return (
      <div className="page-container">
    ...
    <div className="page-container" style={{paddingBottom: '100px'}}>
      ...
      <div className="chart-card">
        <div className="chart-header">
          <h3 className="chart-title">Edit Reward #{id}</h3>
        </div>
      ...
      <div style={{
        ...
        <Link to={`/rewards/${id}`} className="btn btn-secondary">
          <X size={16} />
          Cancel
        </Link>
        <button 
