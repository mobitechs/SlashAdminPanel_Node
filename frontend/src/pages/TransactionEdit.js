...
const fetchTransaction = useCallback(async () => {
  try {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(`⚠️ Transaction endpoint ${endpoint} failed:`, response.status);
        }
      } catch (err) {
        console.warn(`⚠️ Transaction endpoint ${endpoint} error:`, err.message);
        continue;
      }
    }
    ...
    const transaction = transactionData.data?.transaction || transactionData.data;
    ...
    setFormData({
      ...
      vendor_discount: transaction.vendor_discount || 0,
      cashback_used: transaction.cashback_used || 0,
      coupon_discount: transaction.coupon_discount || 0,
      final_amount: transaction.final_amount || '',
      cashback_earned: transaction.cashback_earned || 0,
    ...
    if (transaction.user_first_name || transaction.user_last_name || transaction.user_email) {
      const user = {
    ...
    if (transaction.store_name || transaction.store_email) {
      const store = {
        id: transaction.store_id,
        name: transaction.store_name || '',
        email: transaction.store_email || ''
      };
      setSelectedStore(store);
      setStoreSearch(`${store.name} (${store.email})`);
    }

    // Set selected coupon if coupon data exists
    if (transaction.coupon_code && transaction.coupon_id) {
      const coupon = {
        id: transaction.coupon_id,
        code: transaction.coupon_code,
        name: transaction.coupon_title || transaction.coupon_code,
        discount_amount: transaction.coupon_discount || ''
      };
      setSelectedCoupon(coupon);
      setCouponSearch(`${coupon.code} (${coupon.name || 'coupon'})`);
    }
  ...
  // Handle search input changes (only search if it's a new search, not when pre-populated)
  useEffect(() => {
...
const handleCouponSelect = (coupon) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('🔄 Updating transaction:', submitData);
  ...
  if (error && !originalTransaction) {
    return <ErrorState message={`Failed to load transaction: ${error}`} />;
  }
    ...
    <div className="page-container" style={{paddingBottom: '100px'}}>
      ...
      <div style={{marginBottom: 'var(--spacing-lg)', display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)'}}>
        ...
        <span style={{color: 'var(--text-primary)', fontSize: '0.875rem'}}>
          Edit Transaction #{formData.transaction_number}
        </span>
      </div>
      ...
      <div className="chart-card">
        <div className="chart-header">
          <h3 className="chart-title">Edit Transaction</h3>
        </div>
                  ...
                  <div style={{position: 'relative'}}>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (selectedCoupon && e.target.value !== `${selectedCoupon.code} (${selectedCoupon.discount_type})`) {
