...
const CouponEdit = () => {
  ...
  const fetchCouponData = async () => {
    try {
      setError(null);
      console.log(`🔄 Fetching coupon data for editing, ID: ${id}`);
      ...
      console.log(`📡 Coupon edit response status: ${response.status}`);
      ...
      const data = await response.json();
      console.log('✅ Coupon Edit API Response:', data);

      if (!data.success || !data.data || !data.data.coupon) {
  ...
  return (
    <div className="page-container" style={{paddingBottom: '100px'}}>
      ...
      <div style={{
        ...
        <Link to={`/coupons/${id}`} className="btn btn-secondary">
          <X size={16} />
          Cancel
        </Link>
        <button 
