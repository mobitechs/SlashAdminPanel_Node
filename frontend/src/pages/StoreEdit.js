// pages/StoreEdit.js - Store Edit Form with Optimized Layout
...
const StoreEdit = () => {
  ...
  // Form data state with store fields
  ...
  const handleFileChange = (e, fieldName) => {
    ...
    // Reset removal flag since we're uploading a new file
  ...
  const removeFile = (fieldName) => {
    ...
    if (existingImages[fieldName] && !files[fieldName]) {
      setImagesToRemove(prev => ({
        ...prev,
        [fieldName]: true
      }));
  ...
  const fetchStoreData = async () => {
    try {
      setError(null);
      console.log(`🔄 Fetching store data for editing, ID: ${id}`);
      ...
      console.log(`📡 Store edit response status: ${response.status}`);
      ...
      const data = await response.json();
      console.log('✅ Store Edit API Response:', data);
      ...
      if (!storeData.id || storeData.id != id) {
      ...
      setFormData({
        ...
        sub_category: storeData.sub_category || '',
        latitude: storeData.latitude || '',
        longitude: storeData.longitude || '',
        normal_discount_percentage: storeData.normal_discount_percentage || '',
        vip_discount_percentage: storeData.vip_discount_percentage || '',
        commission_percent: storeData.commission_percent || '',
        minimum_order_amount: storeData.minimum_order_amount || '',
        upi_id: storeData.upi_id || '',
        google_business_url: storeData.google_business_url || '',
        contract_start_date: storeData.contract_start_date ? storeData.contract_start_date.split('T')[0] : '',
        contract_expiry_date: storeData.contract_expiry_date ? storeData.contract_expiry_date.split('T')[0] : '',
        owner_password: '', // Don't pre-fill password for security
        closed_by: storeData.closed_by || '',
        is_premium: storeData.is_premium || 0,
        is_active: storeData.is_active || 1
      });

      // Set existing images - construct full URLs if needed
      const getFullImageUrl = (imagePath) => {
        ...
        return `${API_BASE_URL}/stores/image/${imagePath}`;
      };

      setExistingImages({
        logo: getFullImageUrl(storeData.logo),
        banner_image: getFullImageUrl(storeData.banner_image),
        qr_code: getFullImageUrl(storeData.qr_code)
      });
      
      console.log('✅ Store edit data loaded successfully for store:', storeData.id);

    } catch (error) {
      console.error(`❌ Failed to fetch store ${id} for editing:`, error.message);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Check if we have files to upload or images to remove
      const hasFiles = Object.values(files).some(file => file !== null);
      const hasImagesToRemove = Object.values(imagesToRemove).some(remove => remove === true);

      if (hasFiles || hasImagesToRemove) {
        ...
        // Add information about images to remove
        Object.keys(imagesToRemove).forEach(key => {
          if (imagesToRemove[key]) {
            formDataToSend.append(`remove_${key}`, 'true');
          }
        });

        console.log(`🔄 Updating store ${id} with data and files`);
        ...
        if (updateData.normal_discount_percentage) updateData.normal_discount_percentage = parseFloat(updateData.normal_discount_percentage);
        if (updateData.vip_discount_percentage) updateData.vip_discount_percentage = parseFloat(updateData.vip_discount_percentage);
        if (updateData.commission_percent) updateData.commission_percent = parseFloat(updateData.commission_percent);
        if (updateData.minimum_order_amount) updateData.minimum_order_amount = parseFloat(updateData.minimum_order_amount);

        console.log(`🔄 Updating store ${id} with data:`, updateData);
    ...
    <div className="page-container" style={{paddingBottom: '100px'}}>
      ...
      <div className="chart-card">
        <div className="chart-header">
          <h3 className="chart-title">Edit Store #{id}</h3>
        </div>
              ...
              <div className="form-group">
                ...
                >
                  <input
                    ...
                    style={{display: 'none'}}
                  />
                  {(filePreviews.logo || (existingImages.logo && !imagesToRemove.logo)) ? (
                    <div>
                      <img 
                        src={filePreviews.logo || existingImages.logo} 
                        alt="Logo preview" 
                        style={{
                          maxWidth: '100%',
                          maxHeight: '100px',
                      ...
                      <div style={{display: 'flex', gap: 'var(--spacing-sm)', justifyContent: 'center'}}>
                        ...
                        >
                          {filePreviews.logo ? 'Remove New' : 'Remove'}
                        </button>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="form-group">
                ...
                >
                  <input
                    ...
                    style={{display: 'none'}}
                  />
                  {(filePreviews.banner_image || (existingImages.banner_image && !imagesToRemove.banner_image)) ? (
                    <div>
                      <img 
                        src={filePreviews.banner_image || existingImages.banner_image} 
                        alt="Banner preview" 
                        style={{
                          maxWidth: '100%',
                          maxHeight: '100px',
                      ...
                      <div style={{display: 'flex', gap: 'var(--spacing-sm)', justifyContent: 'center'}}>
                        ...
                        >
                          {filePreviews.banner_image ? 'Remove New' : 'Remove'}
                        </button>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="form-group">
                ...
                >
                  <input
                    ...
                    style={{display: 'none'}}
                  />
                  {(filePreviews.qr_code || (existingImages.qr_code && !imagesToRemove.qr_code)) ? (
                    <div>
                      <img 
                        src={filePreviews.qr_code || existingImages.qr_code} 
                        alt="QR Code preview" 
                        style={{
                          maxWidth: '100%',
                          maxHeight: '100px',
                      ...
                      <div style={{display: 'flex', gap: 'var(--spacing-sm)', justifyContent: 'center'}}>
                        ...
                        >
                          {filePreviews.qr_code ? 'Remove New' : 'Remove'}
                        </button>
                      </div>
                    </div>
                  ) : (
            ...
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--spacing-lg)'}}>
              ...
              <div style={{display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)'}}>
                <input
                  ...
                  checked={parseInt(formData.is_premium) === 1}
                  onChange={handleInputChange}
                />
              ...
              <div style={{display: 'flex', alignItems: 'center', gap: 'var(--spacing-md)'}}>
                <input
                  ...
                  name="is_active"
                  className="checkbox-input"
                  checked={parseInt(formData.is_active) === 1}
                  onChange={handleInputChange}
                />
      ...
      <div style={{
        ...
        <Link to={`/stores/${id}`} className="btn btn-secondary">
          <X size={16} />
          Cancel
        </Link>
        <button 
